...
| 014 | Adicionar skill/requisito faltando direto no currículo (via IA, na aba ATS Match) — toca também `backend_hirefy` | 📝 Spec escrita, investigação concluída (reusa `GET/PUT /resumes` já existentes, só falta o endpoint de sugestão); implementação não iniciada | [spec.md](014-adicionar-skill-faltando/spec.md) |
