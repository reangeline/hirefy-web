...
| 011 | Auto-apply assistido no LinkedIn (extensão de navegador) — novo projeto (`browser_extension/`), toca também `backend_hirefy` | 🚧 Testado ao vivo com login real: "Preparar candidatura" roda ponta a ponta; Wishlist sincronizado entre extensão e Dashboard; gate Premium confirmado (403) com conta Free; reabrir vaga já aprovada agora prepara a candidatura sozinha (envio continua manual). Dois bugs reais de `jobUrl`/estado entre vagas encontrados e corrigidos no processo. Falta testar: sugestão de resposta via IA numa pergunta de triagem real, e o registro no Pipeline depois de um envio de verdade | [spec.md](011-auto-apply-linkedin/spec.md) |
