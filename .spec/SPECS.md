...
| 011 | Auto-apply assistido no LinkedIn (extensão de navegador) — novo projeto (`browser_extension/`), toca também `backend_hirefy` | 🚧 Testado ao vivo com login real: "Preparar candidatura" ponta a ponta; Wishlist sincronizado com o Dashboard; gate Premium confirmado (403); reabrir vaga aprovada prepara sozinha (envio continua manual); sugestão de resposta via IA testada numa pergunta de triagem real. Vários bugs reais encontrados e corrigidos no processo (jobUrl inconsistente, estado vazando entre vagas, painel inclicável com o modal de Easy Apply aberto por causa da top layer do `<dialog>`, raspagem ficando vazia). Falta testar: o registro no Pipeline depois de um envio de verdade | [spec.md](011-auto-apply-linkedin/spec.md) |
| 012 | Prática de entrevista focada nos gaps reais da vaga (diferencial: ir além de conseguir a entrevista, ajudar a passar nela) — toca também `backend_hirefy` | ✅ Implementado, deployado em dev e testado ao vivo (MAVI, 43% ATS): pergunta gerada sondou de propósito o gap real backend-vs-fullstack da vaga, UI avisou o modo "focado nos gaps". Corrigido de brinde um bug real (`resumeDataFor` buscava na tabela errada pra vaga já otimizada) | [spec.md](012-interview-prep-focado-em-gaps/spec.md) |
